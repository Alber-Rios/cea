...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function addMessage(text, sender, showMenu = false, save = true) {
        ...
        // El Smart Scroll se llama directamente desde handleSend
        smartScroll(); 
    }
    ...
    function mostrarMenuOpciones() {
        ...
        // NOTA: No hay scroll forzado aquí para que el chat no salte
    }


    // =================================================================
    // 🕵️‍♀️ LÓGICA ASÍNCRONA DE IA
    // =================================================================
    ...
    async function encontrarRespuesta(input) {
        ...
        for (let key in CONOCIMIENTO_BOT) {
            // Nota: Aquí se asume que las keywords son strings que se buscan dentro del texto.
            if (CONOCIMIENTO_BOT[key].keywords.some(k => text.includes(k))) {
                const info = CONOCIMIENTO_BOT[key];
                contextoBot = key; 
                
                // Si la respuesta es hardcodeada (no dinámica de IA), la devuelve al instante.
        ...
        // 2. LLAMADA AL SERVIDOR DE IA (Appwrite Function - para Eventos y preguntas complejas)
        try {
